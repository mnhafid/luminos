...
`cap auth login` needs an interactive terminal to paste the authorization code into (use `--no-open` to print the URL and open it on another machine). For fully non-interactive runners, mint an API key from the Cap dashboard instead:
